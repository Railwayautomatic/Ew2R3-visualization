...
(async () => {
      ...
      for (const language of languages) {
        ...
        const state = await page.evaluate(({ language, original, route, researchEnglishOnly, publicArticleTitles }) => {
          ...
          return {
            ...
            publicTitlesVisible: route !== 'research' || publicArticleTitles.every(text => document.documentElement.innerHTML.includes(text))
          };
  ...
  await browser.close();
  const failed = results.filter(r => r.status !== 200 || r.htmlLang !== r.language || !r.changed || r.overflow || r.emptyMain || !r.researchTranslated || !r.publicTitlesVisible);
